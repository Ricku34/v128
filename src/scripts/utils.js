...
(async function() {
	...
	window.utils = {
		ready : new Promise(async function(resolve) {

			await v128.init(1);
			var translateMat = v128.matrix.new();
			var rotateXMat = v128.matrix.new();
			var rotateYMat = v128.matrix.new();
			var rotateZMat = v128.matrix.new();
			var scaleMat = v128.matrix.new();
			...
			utils.xformMatrix = function xformMatrix(xform, translate, rotate, scale) {
					...
					rotate = v128.memory.toArray(rotate || utils.ZEROS);
					scale = scale || utils.ONES;

					v128.matrix.fromTranslation(translate, translateMat);
					v128.matrix.fromXRotation(rotate[0], rotateXMat);
					v128.matrix.fromYRotation(rotate[1], rotateYMat);
					v128.matrix.fromZRotation(rotate[2], rotateZMat);
					v128.matrix.fromScaling(scale, scaleMat);

					v128.matrix.multiply(rotateXMat,   scaleMat, xform);
					v128.matrix.multiply(rotateYMat,   xform,    xform);
					v128.matrix.multiply(rotateZMat,   xform,    xform);
					v128.matrix.multiply(translateMat, xform,    xform);
			};

			utils.loadCubemapImages = function loadCubeMapImages(urls, ok) {
				var numImages = 6;
			...
			utils.createBox = function createBox(options) {
				options = options || {};
			...
			utils.createSphere = function createSphere(options) {
				options = options || {};
			...
			utils.computeBoundingBox = function (position, options) {
				options = options || {};
